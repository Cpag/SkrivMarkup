...
class LipsumInlineExtension implements SkrivInlineExtension {
	...
	public function to($type, array $params = null) {
		...
		$nb = intval($params[0]);
		$words = array_slice($this->contentAsWords, 0, $nb);
		if (empty($words))
			return '';
		switch ($type) {
			case 'html':
				return $this->toHtml($words);
			case 'plain-text':
				return $this->toPlainText($words);
			default:
				throw new \Exception('Unknown type "' . $type . '"');
		}
	}
	private function toHtml(array $words) {
		return '<span class="lipsum">' . implode(' ', $words) . '</span>';
	}
	private function toPlainText(array $words) {
